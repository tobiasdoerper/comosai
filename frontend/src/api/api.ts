import { UserInfo, ConversationRequest, Conversation, ChatMessage, CosmosDBHealth, CosmosDBStatus, BlobImage } from "./models";
...
export async function uploadAttachment(blobImage: BlobImage): Promise<BlobImage> {
    try {
        const formData = new FormData();
        formData.append('file', blobImage.file!);
        const response = await fetch('/upload/image', {
            method: 'POST',
            body: formData,
        });
        const result = await response.json();
        let blobImageResult: BlobImage = {};
        blobImageResult.file = blobImage.file;
        blobImageResult.file_id = result.file_id;
        blobImageResult.sas_url = result.sas_url;
        blobImageResult.file_name = result.file_name;
        console.log(response);
        return blobImageResult;
    } catch (error) {
...
export async function getAttachment(file_id: string): Promise<BlobImage> {
    try {
        const response = await fetch('/download/image/' + file_id, {
            method: 'GET'
        });
        const result = await response.json();
        let blobImage: BlobImage = {};
        blobImage.file_id = result.file_id;
        blobImage.sas_url = result.sas_url;
        blobImage.file_name = result.file_name;
        console.log(result.sas_url);
        return blobImage;
    } catch (error) {
        console.error('Error:', error);
        return {};
...
export async function deleteAttachment(file_name: string): Promise<boolean> {
    try {
        const response = await fetch('/delete/image/' + file_name, {
            method: 'GET'
        });
        const result = await response.json();
        console.log("file: " + file_name + " deleted: " + result.deletion);
